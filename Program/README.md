# 记录iOS编程中的点滴

### iOS中常用的网站

- Apple官方
    - [iOS官方人机交互指南](https://developer.apple.com/design/human-interface-guidelines/)
    - [iOS设备尺寸及分辨率](https://developer.apple.com/design/human-interface-guidelines/ios/visual-design/adaptivity-and-layout/)
    - [iOS审核指南](https://developer.apple.com/app-store/review/guidelines/)
    - [开发者登录](https://developer.apple.com/)
    - [iTunes  Connect](https://itunesconnect.apple.com/login)
    - [苹果API文档](https://developer.apple.com/documentation/uikit?language=objc)

- 教程
    - [中文版 Apple 官方 Swift 教程](https://swiftgg.gitbook.io/swift/)
    - [ObjC 中国](https://objccn.io/)



### Runtime的总结
* [Category的实现原理](Program/iOS/OC/Category实现原理.md)
* [isa指针](Program/iOS/OC/isa和Class.md)
* [OC方法调用过程](Program/iOS/OC/OC方法调用过程.md)
* [OC消息机制](Program/iOS/OC/OC消息机制.md)
* [runtime如何实现weak属性](Program/iOS/OC/runtime如何实现weak属性.md)
* [runtime具体应用](Program/iOS/OC/runtime具体应用.md)
* [runtime性能优化](Program/iOS/OC/runtime性能优化.md)

### Runloop的总结
* [触摸事件传递和响应原理](Program/iOS/OC/触摸事件传递和响应原理.md)
* [Runloop与线程](Program/iOS/OC/Runloop与线程.md)
* [iOS事件响应原理](Program/iOS/OC/iOS事件响应原理.md)
